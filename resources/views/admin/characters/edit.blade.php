                ...
                <form action="{{ route('admin.characters.update', $character) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('game_id') is-invalid @enderror" 
                            ...
                            @foreach($games as $game)
                                <option value="{{ $game->id }}" {{ $character->game_id == $game->id ? 'selected' : '' }}>
                                    {{ $game->game_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" 
                               id="name" name="name" required value="{{ $character->name }}">
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex gap-3">
                            @for($i = 5; $i >= 3; $i--)
                                <label class="rarity-option {{ $character->rarity == $i ? 'selected' : '' }}">
                                    <input type="radio" name="rarity" value="{{ $i }}" {{ $character->rarity == $i ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('element_id') is-invalid @enderror" 
                            ...
                            @foreach($elements as $element)
                                <option value="{{ $element->id }}" {{ $character->element_id == $element->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('role_id') is-invalid @endretry('role_id') is-invalid @enderror" 
                                id="role_id" name="role_id">
                            <option value="">-- Pilih Role --</option>
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}" {{ $character->role_id == $role->id ? 'selected' : '' }}>
                                    {{ $role->role_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        @if($character->image_url)
                            <div class="current-image">
                                @if(strpos($character->image_url, '/storage/') === 0 || strpos($character->image_url, '://') !== false)
                                    <img src="{{ $character->image_url }}" alt="{{ $character->name }}">
                                @else
                                    <img src="{{ asset($character->image_url) }}" alt="{{ $character->name }}">
                                @endif
                                <div class="current-image-text">
                                    <strong>Gambar Saat Ini</strong>
                                    <small class="d-block">Klik area di bawah untuk mengubah gambar</small>
                                </div>
                            </div>
                        @endif
