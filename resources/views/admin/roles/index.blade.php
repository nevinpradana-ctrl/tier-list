    ...
    <div class="col-md-9">
        <div class="content-wrapper">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                <h1 class="mb-0">
                    <i class="bi bi-shield me-2"></i>Kelola Role
                </h1>
            ...
            @forelse($roles->groupBy('game_id') as $gameId => $gameRoles)
                <div class="mb-4">
                    <h5 class="mb-3 d-flex align-items-center gap-2">
                        <span class="game-badge">{{ $gameRoles->first()->game->game_name ?? 'N/A' }}</span>
                    </h5>
                    <div class="row g-3">
                            ...
                            <div class="col-md-6 col-lg-4">
                                <div class="role-card h-100">
                                    @if($role->icon_url && strpos($role->icon_url, '/storage/') === 0)
                                        <img src="{{ $role->icon_url }}" alt="{{ $role->role_name }}" class="role-icon">
                                    @elseif($role->icon_url)
                                        <img src="{{ asset($role->icon_url) }}" alt="{{ $role->role_name }}" class="role-icon">
                                    @else
                                        <div class="role-icon d-flex align-items-center justify-content-center">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <small class="text-muted">
                                            {{ \App\Models\Character::where('role_id', $role->id)->count() }} karakter
                                        </small>
                                    </div>
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('admin.roles.edit', $role->id) }}" class="btn btn-sm btn-warning action-btn">
                                        ...
                                        <form action="{{ route('admin.roles.destroy', $role->id) }}" method="POST" style="display: inline;">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-danger action-btn" 
                                                    onclick="return confirm('Hapus role {{ $role->role_name }}?');">
            ...
            @empty
                <div class="empty-state">
                    ...
                    <h5 class="text-muted mb-3">Belum ada role</h5>
                    <p class="text-muted mb-3">Tambahkan role pertama untuk memulai</p>
                    <a href="{{ route('admin.roles.create') }}" class="btn btn-primary">
                        <i class="bi bi-plus-lg me-2"></i>Tambah Role
                    </a>
                </div>
