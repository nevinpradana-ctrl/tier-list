...
<div class="d-flex">
    ...
    <div class="col-md-9">
        <div class="content-wrapper">
            ...
            <h1 class="mb-4">
                <i class="bi bi-pencil-square me-2"></i>Edit Element
            </h1>

            <div class="form-panel">
                <form action="{{ route('admin.elements.update', $element->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('game_id') is-invalid @enderror" 
                            ...
                            @foreach($games as $game)
                                <option value="{{ $game->id }}" {{ $element->game_id == $game->id ? 'selected' : '' }}>
                                    {{ $game->game_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        <label for="element_name" class="form-label">
                            <i class="bi bi-lightning me-2"></i>Nama Element <span class="text-danger">*</span>
                        </label>
                        <input type="text" class="form-control @error('element_name') is-invalid @enderror" 
                               id="element_name" name="element_name" value="{{ $element->element_name }}" required>
                        @error('element_name')
                            <div class="text-danger mt-2"><i class="bi bi-exclamation-circle me-1"></i>{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="form-label">
                            <i class="bi bi-image me-2"></i>Icon Element
                        </label>
                        @if($element->icon_url)
                            <div class="current-icon">
                                @if(strpos($element->icon_url, '/storage/') === 0 || strpos($element->icon_url, '://') !== false)
                                    <img src="{{ $element->icon_url }}" alt="{{ $element->element_name }}">
                                @else
                                    <img src="{{ asset($element->icon_url) }}" alt="{{ $element->element_name }}">
                                @endif
                                <div>
                    ...
                    <div class="btn-group">
                        ...
                        <a href="{{ route('admin.elements.index') }}" class="btn btn-cancel" style="flex: 1;">
