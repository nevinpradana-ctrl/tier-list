                ...
                <form action="{{ route('admin.games.update', $game) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('game_name') is-invalid @enderror" 
                               id="game_name" name="game_name" required value="{{ $game->game_name }}">
                        @error('game_name')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('slug') is-invalid @enderror" 
                               id="slug" name="slug" required value="{{ $game->slug }}">
                    ...
                    <div class="mb-4">
                        ...
                        @if($game->icon_url)
                            <div class="current-icon">
                                @if(strpos($game->icon_url, '/storage/') === 0 || strpos($game->icon_url, '://') !== false)
                                    <img src="{{ $game->icon_url }}" alt="{{ $game->game_name }}">
                                @else
                                    <img src="{{ asset($game->icon_url) }}" alt="{{ $game->game_name }}">
                                @endif
                                <div class="current-icon-text">
                                    <strong>Icon Saat Ini</strong>
                                    <small class="d-block">Klik area di bawah untuk mengubah icon</small>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  id="description" name="description" rows="4">{{ $game->description ?? '' }}</textarea>
                        @error('description')
                            <div class="text-danger mt-2"><i class="bi bi-exclamation-circle me-1"></i>{{ $message }}</div>
                        @enderror
                    </div>
